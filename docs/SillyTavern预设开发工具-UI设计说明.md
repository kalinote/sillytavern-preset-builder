...
> 本文是《SillyTavern预设开发工具-实施方案》的 UI 落地补充。当前工作区已经接入真实工程文件 API、Monaco、CodeMirror 6、自动保存、HTML/CSS 静态预览，以及无需安装扩展的 Node 直连 ST 会话、preset 目录/读取和手动保存；Prompt/Regex 结构化工作台与更多静态诊断仍是后续阶段。直连 REST 不能捕获已打开 ST 页面的最终 Prompt、token、DOM 或 Console，界面不会用演示数据冒充真实运行结果。
...
2. 明确区分“工程已保存”“已导出 JSON”“已保存到 ST”三种状态。
...
- 说明工具与 ST 的职责边界。
- 填写 Node 可访问的 ST HTTP(S) Origin；如果目标启用了 Basic Authentication 或多用户登录，实时填写相应凭据。
- 强调账号密码、ST Cookie 和 CSRF Token 只在 Node 内存会话中使用，不写磁盘或浏览器持久存储；同时警告完整 preset 自身的代理密码、自定义请求头等连接字段会进入工程。
- 连接成功后可以浏览 preset 目录、从所选 preset 创建工程，或执行推送预览/保存。

当前实现默认展示真实连接门禁；会话创建并完成兼容性检查后才进入工作台。顶部连接按钮展示 Origin、ST 版本、账号 handle（如有）、认证方式、最近检查时间和 target policy，并支持检查或主动断开。连接表单不得把密码写入 localStorage、URL、日志或可回显状态。

所有 ST 会话请求使用同源 `/api` 和 `credentials: "same-origin"`。生产部署把前端与 API 分开托管时，必须在前端 Origin 反向代理 `/api`；界面不支持仅靠 CORS 跨站携带 ST 会话 Cookie。
...
┌────────────────────────── 顶部全局工具栏 ──────────────────────────┐
├───────────┬───────────────────────────────┬────────────────────────┤
│ 工程结构树 │ Monaco / 文件编辑区            │ 属性 / 预览 / ST 状态   │
│ 288px     │ 自适应                         │ 360px                  │
└───────────┴───────────────────────────────┴────────────────────────┘
```
...
4. **ST**：连接状态、preset 目录、保存状态，以及跳转到 ST 手动选择/运行的说明。

底部导航固定显示，结构列表选择文件后自动返回编辑视图。顶部将导出、保存到 ST 和连接等低频操作收进更多菜单。

## 5. 关键界面与交互

### 5.1 顶部工具栏

顶栏从左到右为：产品标识、工程名称与版本、自动保存状态、ST 连接状态、导出、保存到 ST、面板开关。

状态语义必须稳定：

- “工程已保存”仅表示 Docker 工作区已落盘。
- “导出 JSON”是显式构建操作，不改变 ST。
- “保存到 ST”是显式远端操作，不开启自动同步，也不切换已经打开的 ST 页面。
...
- 只有先保存 preset、再在 ST 页面刷新并手动选择目标后，脚本才在 ST 的真实扩展环境中执行。
...
明确禁用：`script`、inline handler、`javascript:` URL、Tavern Helper、`parent/top` 与 ST API。完整交互使用“保存到 ST”，随后在 ST 页面手动选择目标并运行。

### 5.7 ST 连接与手动运行

ST 面板展示：

- ST Origin、版本、账号 handle（如有）、兼容性、认证方式和 target policy；
- 会话检查、断开与重新连接；
- Chat Completion preset 目录和明确选择；
- 最近一次推送预览/保存状态；
- “打开 ST”及刷新列表、手动选择目标、执行真实测试的步骤。

第一版不展示 Prompt 捕获、真实 token、最终消息树、DOM 快照或 Console，因为 ST 的服务端 preset API 不提供某个已打开页面的运行事件。缺少数据时显示准确的能力边界，而不是空造运行结果。

### 5.8 手动保存确认

保存弹窗在真正写入 ST 前展示：

- ST 当前版本、目标存在性以及工程/远端 revision 与体积；
- created/changed/unchanged 总体变化类别；
- 构建 diagnostics、Regex 镜像和未知字段校验结果；
- 目标操作：创建新 preset 或覆盖已有 preset；两种模式都必须满足对应的目标存在性条件。
- 预览的短时有效期，以及工程或远端变化后必须重新预览的提示。

确认提交只接受服务端返回的单次 preview token，不由浏览器重新提交任意 preset JSON。完成保存后，后续编辑仍只自动保存工程，不自动同步到 ST；成功态文案必须是“已保存到 ST，请在 ST 刷新并手动选择”，不能写成“已应用”。

当前纵切片只承诺总体 hash/体积/diagnostics 预览；Prompt、Regex、Script 的字段级可视化 diff 属于后续 Diff Viewer，不得在尚未计算时伪装成完整差异。
...
- 390×844：连接门禁、结构、编辑和预览布局无横向溢出；ST 页面运行时捕获明确标为不支持，不展示模拟结果。
- 手机底部导航单项约 96×63px，满足触控操作空间。
- 保存确认弹窗在 1280×720 视口完整显示。
...
3. 完成 preset create/overwrite 推送预览、短时确认 token 和保存结果 UI。
4. 完成 Docker 实镜像，以及无认证、Basic、多用户登录和 LAN ST 连接验收。
5. 完成 PWA、真实 ST 1.18.x 和多视口性能验收；页面运行时捕获作为后续独立方案评估。
